...
class WaymaxEnvironment(waymax_env.BaseEnvironment):
    def __init__(
        ...
        if log_replay:
            ...
            # Create a replay actor for agents not controlled by UniDBO.
            self.nc_actor = agents.create_expert_actor( 
                dynamics_model=dynamics_model,
                is_controlled_func=lambda state: state.object_metadata.is_valid)
    ...
    def step_sim_agent(
        ...
        """
        # Step the replay policy for agents not controlled by UniDBO.
        nc_action_full: datatypes.Action = self.jit_nc_action({}, current_state, None, None)
