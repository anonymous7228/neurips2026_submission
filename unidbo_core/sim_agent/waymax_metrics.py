...
class WrongWayMetric(abstract_metric.AbstractMetric):
    ...
    def compute(
        ...
        speed = jnp.hypot(current_object_state.vel_x, current_object_state.vel_y)

        distance = jnp.linalg.norm(current_object_state.xy[..., jnp.newaxis, :] - roadgraph_points.xy, axis=-1)
        is_road_lane = jnp.logical_or(roadgraph_points.types == MapElementIds.LANE_FREEWAY,
        ...
        for i in range(current_object_state.shape[0]):
            if not is_vehicle[i]:
                ...
                nearby_points_yaw = jnp.arctan2(nearby_points_dir[:, 1], nearby_points_dir[:, 0])
                heading_error = jnp.abs(nearby_points_yaw - current_object_state.yaw[i])
                heading_error = jnp.minimum(heading_error, 2 * jnp.pi - heading_error)
                if heading_error.min() > self.WRONG_WAY_THRES:
                    wrong_way = wrong_way.at[i].set(1.0)

        valid = jnp.ones_like(wrong_way, dtype=jnp.bool_)

        return abstract_metric.MetricResult.create_and_validate(wrong_way, valid)
...
class OffroadMetric(abstract_metric.AbstractMetric):
    ...
    def compute(
        ...
        speed = jnp.hypot(current_object_state.vel_x, current_object_state.vel_y).squeeze()
        is_vehicle = simulator_state.object_metadata.object_types == 1
...
def compute_signed_distance_to_nearest_road_edge_point(
    ...
    # Shape: (num_points, 2).
    previous_vector_xys = roadgraph_points.dir_xyz[previous_indices, :2]
    ...
    cross_product_previous = jnp.cross(points_to_edge, previous_vector_xys)
    # If the previous point is contiguous, consider both half-plane distances.
    # Shape: (num_points).
    previous_point_in_same_curve = jnp.equal(
        roadgraph_points.ids[nearest_indices], roadgraph_points.ids[previous_indices]
    )
    offroad_sign = jnp.sign(
        jnp.where(
            jnp.logical_and(
                previous_point_in_same_curve, cross_product_previous < cross_product
            ),
            cross_product_previous,
            cross_product,
        )
    )
    # Shape: (num_query_points).
