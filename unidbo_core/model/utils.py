...
class DHNNoiseScheduler(torch.nn.Module):
    def __init__(
...
class DHNDeterministicScheduler(DHNNoiseScheduler):
    def __init__(self, steps=100, schedule='cosine', clamp_val: float = 5.0):
        super().__init__(steps, schedule, clamp_val)
        
    def copy_from_dhn(self, scheduler: DHNNoiseScheduler):
        self.num_steps = scheduler.num_steps
        self.schedule = scheduler.schedule
        self.clamp_val = scheduler.clamp_val
        
        self.register_buffer('betas', scheduler.betas.clone())
        self.register_buffer('betas_sqrt', scheduler.betas_sqrt.clone())
        self.register_buffer('alphas', scheduler.alphas.clone())
        self.register_buffer('alphas_cumprod', scheduler.alphas_cumprod.clone())
    ...
    def step(
        ...
        if not isinstance(timesteps, int):
            while len(timesteps.shape) < len(model_output.shape):
                timesteps = timesteps.unsqueeze(-1)
        # Compute predicted previous sample µ_t-1
        pred_prev_sample_mean = self.q_mean(model_output, timesteps, sample, eta=eta)
