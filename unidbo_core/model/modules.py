...
class Encoder(nn.Module):
    def __init__(self, layers=6, model_dim=256, num_heads=8, dropout=0.1):
        super().__init__()
        self._model_dim = model_dim
        self.agent_encoder = AgentEncoderV2(model_dim=model_dim)
        self.map_encoder = MapEncoder(model_dim=model_dim)
        self.traffic_light_encoder = TrafficLightEncoder(model_dim=model_dim)
        self.relation_encoder = FourierEmbedding(input_dim=3, hidden_dim=model_dim)
    ...
    def forward(self, inputs, record_attn: bool = False):
        # agent encoding
        ...
        # encoded agent tokens (per agent feature after motion/type encoders)
        encoder_outputs['agent_tokens'] = encoded_agents
        ...
        encodings, attn_out_list, attn_w_list = self.transformer_encoder(
            relations, encoded_agents, encoded_map_lanes, encoded_traffic_lights,
            agents_mask, maps_mask, traffic_lights_mask,
            record_attn=record_attn,
        )
        encoder_outputs['encodings'] = encodings
        if record_attn:
            encoder_outputs['attention_values'] = attn_out_list
            encoder_outputs['attention_weights'] = attn_w_list
...
class Denoiser(nn.Module):
    def __init__(
        ...
        self._noise_condition = noise_condition
        self._model_dim = model_dim
        mid_dim = max(64, model_dim // 2)
        self.noise_level_embedding = nn.Embedding(steps, model_dim)
        self.sigma_encoder = nn.Sequential(
            nn.Linear(1, mid_dim),
            nn.SiLU(),
            nn.Linear(mid_dim, model_dim),
        )
    ...
    def forward(self, encoder_inputs, noisy_actions, diffusion_step, rollout = True):
        ...
        elif isinstance(diffusion_step, float):
            diffusion_step = torch.full(
                noisy_actions.shape[:-2], diffusion_step,
                dtype=torch.float32, device=noisy_actions.device
            )
        elif torch.is_tensor(diffusion_step):
            diffusion_step = diffusion_step.to(noisy_actions.device)
            if diffusion_step.ndim > 2:
                diffusion_step = diffusion_step.reshape(*diffusion_step.shape[:2], -1)[..., 0]
        else:
            raise TypeError(f"Unsupported diffusion_step type: {type(diffusion_step)}")
        ...
        # denoise condition
        use_continuous = torch.is_floating_point(diffusion_step) or self._noise_condition == 'continuous'
        if use_continuous:
            sigma = diffusion_step.float().clamp(min=1e-6)
            noise_level = self.sigma_encoder(torch.log(sigma).unsqueeze(-1))
        else:
            noise_level = self.noise_level_embedding(diffusion_step.long())
        if rollout:
...
class AgentEncoderV2(nn.Module):
    def __init__(self, model_dim=256):
        ...
        self.type_embed = nn.Embedding(4, model_dim, padding_idx=0)
...
class MapEncoder(nn.Module):
    def __init__(self, model_dim=256):
        super().__init__()
        self.point = nn.Sequential(nn.Linear(3, 128), nn.ReLU(), nn.Linear(128, model_dim))
        self.traffic_light_embed = nn.Embedding(8, model_dim)
        self.type_embed = nn.Embedding(21, model_dim, padding_idx=0)
...
class TrafficLightEncoder(nn.Module):
    def __init__(self, model_dim=256):
        super().__init__()
        self.type_embed = nn.Embedding(8, model_dim)
...
class QCMHA(nn.Module):
    ...
    def forward(self, query, rel_pos, attn_mask=None, return_attn: bool = False):
        '''
        ...
        attn_probs = self.dropout(dot_score)

        value = torch.matmul(attn_probs, v)
        ...
        if rel_pos is not None:
            ...
            value_rel = torch.matmul(attn_probs.unsqueeze(-2), rel_pos_v).squeeze(-2)
            value += value_rel
...
class SelfTransformer(nn.Module):
    def __init__(self, dim=256, num_heads=8, dropout=0.1):
        super().__init__()
        self.qc_attention = QCMHA(dim, num_heads, dropout)
        self.norm_1 = nn.LayerNorm(dim)
        self.norm_2 = nn.LayerNorm(dim)
        self.ffn = nn.Sequential(nn.Linear(dim, dim*4), nn.GELU(), nn.Dropout(dropout), 
                                 nn.Linear(dim*4, dim), nn.Dropout(dropout))

    def forward(self, inputs, relations, mask=None, record_attn: bool = False):
        if record_attn:
            attention_output, attn_w = self.qc_attention(inputs, relations, mask, return_attn=True)
        else:
        ...
        return output, attn_w, attention_output
...
class TransformerEncoder(nn.Module):
    def __init__(self, layers=6, model_dim=256, num_heads=8, dropout=0.1):
        ...
        self.layers = nn.ModuleList(
            [SelfTransformer(dim=model_dim, num_heads=num_heads, dropout=dropout) for _ in range(layers)]
        )

    def forward(self, encoded_relations, encoded_trajs, encoded_polylines, encoded_traffic_lights,
                trajs_mask, polylines_mask, traffic_lights_mask, record_attn: bool = False):
        ...
        for layer in self.layers:
            encodings, attn_w, attn_out = layer(encodings, encoded_relations, attention_mask, record_attn=record_attn)
            if record_attn:
                attn_out_list.append(attn_out)
                attn_w_list.append(attn_w)

        if record_attn:
            return encodings, attn_out_list, attn_w_list
        return encodings, [], []
...
class CrossTransformer(nn.Module):
    def __init__(self, dim=256, num_heads=8, dropout=0.1):
        super().__init__()
        self.cross_attention = nn.MultiheadAttention(dim, num_heads, dropout, batch_first=True)
        self.norm_1 = nn.LayerNorm(dim)
        self.norm_2 = nn.LayerNorm(dim)
        self.ffn = nn.Sequential(nn.Linear(dim, dim*4), nn.GELU(), nn.Dropout(dropout), 
                                 nn.Linear(dim*4, dim), nn.Dropout(dropout))
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.time_embedding = nn.Embedding(self._future_len, model_dim)
        self.attention_layers = nn.ModuleList(
            [CrossTransformer(dim=model_dim, num_heads=num_heads, dropout=dropout) for _ in range(4)]
        )
        self.encoder = nn.Sequential(nn.Linear(self._input_dim, 128), nn.ReLU(), nn.Linear(128, model_dim))
        self.decoder = nn.Sequential(nn.Linear(model_dim, mid_dim), nn.ELU(), nn.Dropout(dropout), nn.Linear(mid_dim, self._output_dim))
    ...
    def forward(self, noisy_trajectories, noise_level, encodings, relations, mask):
        ...
        for i in range(self._agents_len):
            query_content = self.attention_layers[0](
                query[:, i], 
                query.reshape(-1, self._agents_len*self._future_len, self._model_dim), 
        ...
        for i in range(self._agents_len):
            query_content = self.attention_layers[2](
                ...
                query_content_stack.reshape(-1, self._agents_len*self._future_len, self._model_dim),
                relations[:, i, :self._agents_len].repeat_interleave(self._future_len, dim=1),
                attn_mask=self.casual_mask[i]) # [B, T, 256]
