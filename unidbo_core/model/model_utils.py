...
def batch_transform_trajs_to_local_frame(trajs, ref_idx=-1):
    """
    Batch transform trajectories to a local frame.

    Args:
        trajs (torch.Tensor): Trajectories tensor of shape [B, N, T, x].
        ref_idx (int): Anchor index for the local frame. Default is -1.
...
def batch_transform_polylines_to_local_frame(polylines):
    """
    Batch transform polylines to a local frame.
...
def batch_transform_trajs_to_global_frame(trajs, current_states):
    """
    Batch transform trajectories to a global frame.
...
def roll_out(
        ...
        Args:
            ...
            global_frame (bool): Flag indicating whether to use the global frame. Default is False.
