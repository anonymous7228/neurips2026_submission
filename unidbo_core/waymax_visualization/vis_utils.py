...
def plot_traj_with_speed(
    ...
    for a in range(A):
        ...
        # override config
        linewidth = 1.4 if fixed_linewidth is None else fixed_linewidth
        linestyle = '-' if fixed_linestyle is None else fixed_linestyle
        alpha = 0.8 if fixed_alpha is None else fixed_alpha

        lc = LineCollection(segments, cmap='plasma', norm=norm, linestyle=linestyle, alpha=alpha, zorder=3)
        # Set the values used for colormapping
        lc.set_array(speed)
        lc.set_linewidth(linewidth)
        line = ax.add_collection(lc)
    if show_colorbar and line is not None:
