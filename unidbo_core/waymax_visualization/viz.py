...
road_type_dict = {
  ...
  # Compatible with enum variants across dataset versions (some start RoadLine at 4).
  4:  'RoadLine-BrokenSingleWhite',
  5:  'RoadLine-SolidSingleWhite',
  ...
  14: 'RoadEdge-Boundary',
  15: 'RoadEdge-Boundary',
  16: 'RoadEdge-Median',
  # Compatible with enum variants across dataset versions (StopSign/Crosswalk/SpeedBump/Driveway).
  17: 'StopSign',
  18: 'Crosswalk',
  19: 'SpeedBump',
  20: 'Driveway',
...
def _plot_bounding_boxes(
    ...
    box_scale: float = 1.0,
    force_overlap_mask: Optional[np.ndarray] = None,
) -> None:
  ...
  # Keep overlap check on original physical box size.
  traj_5dof_overlap = traj_5dof

  # Apply optional visual scaling for rendered boxes.
  traj_5dof_vis = traj_5dof.copy()
  if float(box_scale) != 1.0:
    traj_5dof_vis[..., 2:4] *= max(0.1, float(box_scale))

  # Calculate current overlap
  # (A, A)
  overlap_mask_matrix = np.asarray(overlap_fn(traj_5dof_overlap[:, time_idx]))
  ...
  if force_overlap_mask is not None:
    force_mask = np.asarray(force_overlap_mask).astype(bool).reshape(-1)
    if force_mask.shape[0] != overlap_mask.shape[0]:
      if force_mask.shape[0] < overlap_mask.shape[0]:
        pad = np.zeros((overlap_mask.shape[0] - force_mask.shape[0],), dtype=bool)
        force_mask = np.concatenate([force_mask, pad], axis=0)
      else:
        force_mask = force_mask[:overlap_mask.shape[0]]
    overlap_mask = overlap_mask | force_mask
  
  
  # Shrinks bounding_boxes for non-current steps.
  traj_5dof_vis[time_indices != time_idx, 2:4] /= 10
  
  # Plot current bounding boxes
  plot_numpy_bounding_boxes(
      ax=ax,
      bboxes = traj_5dof_vis[:, time_idx][valid_controlled[:, time_idx] & ~overlap_mask],
      color=COLOR_DICT['controlled'],
  )  
  
  plot_numpy_bounding_boxes(
      ax=ax,
      bboxes=traj_5dof_vis[(time_indices == time_idx) & valid_context],
  ...
  if is_ego is not None:
    plot_numpy_bounding_boxes(
        ax=ax,
        bboxes=traj_5dof_vis[is_ego, time_idx],
  ...
  if is_adv is not None:
    plot_numpy_bounding_boxes(
        ax=ax,
        bboxes=traj_5dof_vis[is_adv, time_idx],
        color=COLOR_DICT['adv'],
    )
  # overlap_mask[:2] = 1
  plot_numpy_bounding_boxes(
    ax=ax,
    bboxes=traj_5dof_vis[:, time_idx][overlap_mask & valid[:, time_idx]],
...
def plot_trajectory(
    ...
    is_ego = None,
    is_adv = None,
    speed_vmin: float = 0.0,
    speed_vmax: float = 20.0,
    traj_linewidth: Optional[float] = None,
    traj_alpha: Optional[float] = None,
    box_scale: float = 1.0,
    force_overlap_mask: Optional[np.ndarray] = None,
) -> None:
  ...
  if start_idx < end_idx:
    ...
    plot_traj_with_speed(
      ...
      ax = ax,
      v_min = float(speed_vmin),
      v_max = float(speed_vmax),
      fixed_linewidth=traj_linewidth,
      fixed_alpha=traj_alpha,
...
def plot_roadgraph_points(
  ...
  for id in rg_ids_unique:
    mask = rg_ids == id
    xy_cur = xy[mask]
    rg_type_key = int(rg_type[mask][0])
    rg_type_cur = road_type_dict.get(rg_type_key, 'RoadLine-Unknown')
    ...
    if main_type == 'RoadLine':
      ...
      # Skip unsupported types (e.g., Driveway / Unknown) to avoid visualization interruption.
      continue
